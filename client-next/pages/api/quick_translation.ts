...
const BACKEND_URL = `${API_BASE}/api/quick_translation`;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === 'POST') {
      console.log('🔄 [API] Handling POST /api/quick_translation');
    ...
    console.error('❌ [API] Quick translation proxy error:', err?.response?.status, err?.message);
    if (err.response) {
